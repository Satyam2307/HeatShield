"""
Census ACS Community Vulnerability Indicators module for Hartford, CT bus stops.
Attaches tract/block-group level indicators:
...
def estimate_census_vulnerability(lat: float, lon: float) -> Dict[str, Any]:
    """Assign Census tract and ACS indicators based on spatial location in Hartford."""
    seed = int(abs(lat * 3000 + lon * 3000)) % 10000
    rng = np.random.RandomState(seed)
    
    # Pick matching tract based on lat/lon region
    if lat > 41.78:
        tract = HARTFORD_TRACTS[1] # Upper Albany / North End
    elif lat < 41.74:
        tract = HARTFORD_TRACTS[3] # South End
    elif lon < -72.70:
        tract = HARTFORD_TRACTS[5] # West End
    elif lon > -72.67:
        tract = HARTFORD_TRACTS[6] # Downtown
    elif lat > 41.76:
        tract = HARTFORD_TRACTS[4] # Asylum Hill
    else:
        tract = HARTFORD_TRACTS[2] # Frog Hollow
    ...
    # Combined vulnerability raw indicator score (0 - 100 scale)
    # Higher zero vehicle %, lower income, higher density = higher vulnerability
